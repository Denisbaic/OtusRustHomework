...
impl Room {
    ...
    pub fn remove_device(&mut self, device_name: &str) -> Option<Box<dyn Device>> {
        let remove_pos = self
            .devices
            .iter()
            .position(|device| device.get_device_name() == device_name);
    ...
    pub fn contains_device(&self, device_name: &str) -> bool {
        self.devices
            .iter()
            .any(|device| device.get_device_name() == device_name)
    }

    pub fn get_device(&self, device_name: &str) -> Option<&dyn Device> {
        self.devices
            .iter()
            .find(|device| device.get_device_name() == device_name)
            .map(|device| &**device)
    }

    pub fn get_device_mut<'a>(&'a mut self, device_name: &str) -> Option<&'a mut dyn Device> {
        for device in self.devices.iter_mut() {
            if device.get_device_name() == device_name {
        ...
        for (room_name, device_name) in room_name_device_name {
            match self.get_room(room_name) {
                Some(room) => match room.get_device(device_name) {
                    Some(device) => match device.create_report() {
                        Ok(report) => {
